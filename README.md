...
- определяет новую базу по первому появлению телефона в Битриксе;
...
Первый запуск без кэша подтягивает полный текущий месяц. Следующие запуски берут Битрикс за текущий месяц, а Скорозвон только за сегодняшний день, чтобы не упираться в rate limit. Локальная база отчета хранится в GitHub Actions cache, а не на компьютере.
